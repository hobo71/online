...
L.Control.ColumnGroup = L.Class.extend({
	...
	drawGroupControl: function (group) {
		...
		if (!group.hidden && endX > startX) {
			...
			this.context.moveTo(startX + this._groupHeadSize, startY + 2 * this.dpiScale);
			this.context.lineTo(endX - this.dpiScale, startY + 2 * this.dpiScale);
			this.context.lineTo(endX - this.dpiScale, startY + 2 + this._groupHeadSize / 2);
			this.context.stroke();
			// draw '-'
	...
	drawLevelHeader: function (level) {
		...
		var startX = (this._cornerHeaderWidth - ctrlHeadSize) * 0.5;
