...
L.Control.RowGroup = L.Class.extend({
	...
	drawGroupControl: function (group) {
		...
		if (startY > this._cornerHeaderHeight) {
			...
			this.context.lineWidth = 0.5 * this.dpiScale;
			this.context.strokeRect(startX, startY, this._groupHeadSize, this._groupHeadSize);
		}

		if (!group.hidden && endY > startY) {
			//draw tail
			this.context.strokeStyle = 'black';
			this.context.lineWidth = 1.5 * this.dpiScale;
			this.context.beginPath();
			this.context.moveTo(startX + 2 * this.dpiScale, startY + this._groupHeadSize);
			this.context.lineTo(startX + 2 * this.dpiScale, endY - this.dpiScale);
			this.context.lineTo(startX + 2 + this._groupHeadSize / 2, endY - this.dpiScale);
			this.context.stroke();
			// draw '-'
			this.context.lineWidth = this.dpiScale;
			this.context.strokeRect(startX + this._groupHeadSize / 4, startY + this._groupHeadSize / 2, this._groupHeadSize / 2, this.dpiScale);
		}
		else if (endY > startY) {
			// draw '+'
			this.context.lineWidth = this.dpiScale;
			this.context.beginPath();
			this.context.moveTo(startX + this._groupHeadSize / 4, startY + this._groupHeadSize / 2);
			this.context.lineTo(startX + 3 * this._groupHeadSize / 4, startY + this._groupHeadSize / 2);
			this.context.moveTo(startX + this._groupHeadSize / 2, startY + this._groupHeadSize / 4);
			this.context.lineTo(startX + this._groupHeadSize / 2, startY + 3 * this._groupHeadSize / 4);
			this.context.stroke();
		}
	},
	...
	drawLevelHeader: function (level) {
		...
		var startY = (this._cornerHeaderHeight - ctrlHeadSize) * 0.5;

		ctx.fillStyle = this._backgroundColor;
		ctx.fillRect(startX, startY, ctrlHeadSize, ctrlHeadSize);
		ctx.strokeStyle = 'black';
		ctx.lineWidth = 0.5;
		ctx.strokeRect(startX, startY, ctrlHeadSize, ctrlHeadSize);
		// draw level number
		ctx.fillStyle = this._textColor;
		ctx.font = this._font.getFont();
		ctx.textAlign = 'center';
		ctx.textBaseline = 'middle';
		ctx.fillText(level + 1, startX + (ctrlHeadSize / 2), startY + (ctrlHeadSize / 2));
	},
