...
L.Control.ColumnHeader = L.Control.Header.extend({
	...
	drawHeaderEntry: function (entry, isOver, isHighlighted, isCurrent) {
		...
		var startY = this.size[1] - this._headerHeight;
		...
		if (isHighlighted) {
			selectionBackgroundGradient = this.context.createLinearGradient(startX, startY, startX, startY + height);
		...
		if (this._headerHeight !== this._canvasHeight) {
			...
			this.context.fillRect(startX, startY - this._borderWidth, width, this._borderWidth);
		}
		...
		if (isCurrent && width > 2 * handleSize) {
			...
			var y = startY + 2 * this.dpiScale;
			var h = height - 4 * this.dpiScale;
		...
		this.context.fillText(content, endX - (width / 2), startY + (height / 2) + 1);
		// draw row separator
		this.context.fillStyle = this._borderColor;
		this.context.beginPath();
		this.context.fillRect(endX -1, startY, this._borderWidth, height);
	},
	...
	onResize: function () {
		var offset = Math.round((this.myTopLeft[1] + this.size[1]) / this.dpiScale);
		this._map.options.documentContainer.style.top = String(offset + 117) + 'px';
		document.getElementsByClassName('leaflet-canvas-container')[0].style.top = String(offset * -1) + 'px';
	},
});
